 ...
 *
 * editRecord() writes with a bare UPDATE ... WHERE id = ?, so the test runs
 * against a real in-memory database and asserts the foreign row is left intact.
 */
class RecordManagerEditRecordOwnershipTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        // Tables UserManager::verifyPermission() and verifyUserIsOwnerZoneId() walk.
    ...
    public function testForgedZoneIdCannotEditForeignRecord(): void
    {
        ...
        $this->assertFalse(
            $this->attemptForgedEdit(self::VICTIM_RECORD_ID, 'pwned.attacker.example'),
            'editRecord must reject a record whose real zone the caller does not own, even when the request carries an owned zid.'
        );

        $stmt = $this->db->query("SELECT content, name FROM records WHERE id = " . self::VICTIM_RECORD_ID);
    ...
    {
        $this->assertFalse($this->attemptForgedEdit(9999, 'ghost.attacker.example'));
    }

    /**
     * Drive editRecord() as the attacker with a request that always claims the
     * attacker's own zone id, and return editRecord()'s result.
     */
    private function attemptForgedEdit(int $rid, string $name): bool
    {
        ...
        return $this->makeRecordManager()->editRecord($record);
    }

    private function makeRecordManager(): RecordManager
    {
        $config = $this->primeConfig();

        // Validation is stubbed to succeed so that, on vulnerable code, nothing
        // but the ownership check stands between the forged request and the UPDATE.
