...
class DomainManagerDeletePermissionTest extends TestCase
{
    ...
    {
        $this->db = new PDOCommon('sqlite::memory:');
    ...
    public function testDeleteRefusedForOwnerWithOnlyEditPermission(): void
    {
        ...
        $this->assertFalse(
            $this->makeDomainManager()->deleteDomain(self::ZONE_DOMAIN_ID),
            'A zone owner holding only zone_content_edit_own must not be able to delete the zone.'
        );
        $this->assertTrue($this->zoneStillExists(), 'The zone row must survive a refused deletion.');
    }
    ...
    public function testDeleteRefusedForNonPrivilegedUser(): void
    {
        ...
        $this->assertFalse($this->makeDomainManager()->deleteDomain(self::ZONE_DOMAIN_ID));
        $this->assertTrue($this->zoneStillExists());
    }
    ...
    public function testDeleteAllowedForOwnerWithDeleteOwn(): void
    {
        ...
        $this->assertTrue($this->makeDomainManager()->deleteDomain(self::ZONE_DOMAIN_ID));
        $this->assertFalse($this->zoneStillExists(), 'The zone row must be removed on an allowed deletion.');
    }
    ...
    public function testDeleteAllowedForNonOwnerWithDeleteOthers(): void
    {
        ...
        $this->assertTrue($this->makeDomainManager()->deleteDomain(self::ZONE_DOMAIN_ID));
        $this->assertFalse($this->zoneStillExists());
    }

    private function zoneStillExists(): bool
    {
        $stmt = $this->db->prepare("SELECT COUNT(*) FROM zones WHERE domain_id = :did");
        $stmt->execute([':did' => self::ZONE_DOMAIN_ID]);
    ...
    private function seedZoneOwnedBy(int $userId): void
    {
        ...
        $this->db->exec("INSERT INTO domains (id, name) VALUES (" . self::ZONE_DOMAIN_ID . ", 'example.com')");
    }
    ...
    private function makeDomainManager(): DomainManager
    {
        ...
        return new DomainManager($this->db, $config, $soa, $repo);
    }
