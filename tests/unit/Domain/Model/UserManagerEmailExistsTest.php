...
class UserManagerEmailExistsTest extends TestCase
{
    ...
    public function testReturnsFalseWhenEmailIsFree(): void
    {
        ...
        $this->assertFalse(UserManager::emailExists($db, 'free@example.com'));
    }
    ...
    public function testExcludesTheEditedUser(): void
    {
        ...
        $db->expects($this->once())
            ->method('prepare')
            ->with($this->stringContains('id != :exclude_id'))
            ->willReturn($stmt);

        $this->assertFalse(UserManager::emailExists($db, 'self@example.com', 42));
    }
}
