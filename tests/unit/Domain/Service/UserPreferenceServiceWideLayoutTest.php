...
class UserPreferenceServiceWideLayoutTest extends TestCase
{
    private function makeService(
        ...
        $config->method('get')->willReturnCallback(
            function (string $section, string $key, $default = null) use ($configDefault) {
                if ($section === 'interface' && $key === 'wide_layout') {
    ...
    #[Test]
    public function testDefaultsToFalseWhenNoStoredPreferenceAndConfigOff(): void
    {
        ...
        $this->assertFalse($service->getWideLayout(42));
    }
    ...
    public function testDefaultsToConfigTrueWhenNoStoredPreference(): void
    {
        ...
        $this->assertTrue($service->getWideLayout(42));
    }

    #[Test]
    public function testStoredTrueOverridesConfigFalse(): void
    {
        $stored = new UserPreference(1, 42, UserPreference::KEY_WIDE_LAYOUT, 'true');
        $repo = $this->createMock(UserPreferenceRepositoryInterface::class);
    ...
    {
        $stored = new UserPreference(1, 42, UserPreference::KEY_WIDE_LAYOUT, 'false');
        $repo = $this->createMock(UserPreferenceRepositoryInterface::class);
        $repo->method('findByUserIdAndKey')->willReturn($stored);
    ...
    #[Test]
    public function testKeyIsAccepted(): void
    {
        $this->assertTrue(UserPreference::isValidKey(UserPreference::KEY_WIDE_LAYOUT));
    }
}
