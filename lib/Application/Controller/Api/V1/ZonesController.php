...
class ZonesController extends PublicApiController
{
    ...
    private function createZone(): JsonResponse
    {
        try {
            ...
            $slaveMaster = $this->inputString($input, 'master', '');
            $enableDnssec = $this->inputBool($input, 'enable_dnssec', false);
            if ($domain === null || $type === null || $slaveMaster === null || $enableDnssec === null) {
            ...
            if ($enableDnssec && $dnssecEnabledServerSide && !$this->permissionService->canManageDnssecForNewZone($userId, $owner)) {
