...
class DomainManager implements DomainManagerInterface
{
    ...
    public function updateZoneRecords(string $db_type, int $dns_ttl, int $zone_id, int $zone_template_id): void
    {
        ...
        if ($zone_template_id != 0) {
            if ($perm_edit == "all" || ($perm_edit == "own" && $user_is_zone_owner == "1")) {
                // Delete the template-applied records. Only MySQL can drop the record
                // and its mapping in one statement; the other backends delete records
                // here and the mapping is cleared uniformly below.
                if ($db_type == 'pgsql') {
