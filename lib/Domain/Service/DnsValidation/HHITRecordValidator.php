 ...
 *
 * HHIT is one of the DRIP (Drone Remote ID Protocol) DNS record types supported
 * by PowerDNS Authoritative 5.1+. The RDATA is an opaque binary blob represented
 * in zone-file form as base64.
 */
class HHITRecordValidator implements DnsRecordValidatorInterface
{
    ...
    public function validate(string $content, string $name, mixed $prio, $ttl, int $defaultTTL, ...$args): ValidationResult
    {
        ...
        $contentResult = Base64BlobValidator::validate($content, _('HHIT record content must be a non-empty base64-encoded blob.'));
        ...
        if ($prio !== null && $prio !== '' && !(is_numeric($prio) && (int) $prio === 0)) {
            return ValidationResult::failure(_('Priority must be 0 for HHIT records.'));
        }
