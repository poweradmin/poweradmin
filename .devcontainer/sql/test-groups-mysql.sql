...
INSERT INTO `log_groups` (`event`, `priority`, `group_id`)
SELECT CONCAT('client_ip:127.0.0.1 user:admin operation:add_members group:Zone_Managers group_id:', g.`id`, ' count:1 members:manager'), 1, g.`id`
FROM `user_groups` g
WHERE g.`name` = 'Zone Managers'
  AND NOT EXISTS (
...
INSERT INTO `log_groups` (`event`, `priority`, `group_id`)
SELECT CONCAT('client_ip:127.0.0.1 user:admin operation:add_zones group:Editors group_id:', g.`id`, ' count:1 zones:shared-zone.example.com'), 1, g.`id`
FROM `user_groups` g
...
INSERT INTO `log_groups` (`event`, `priority`, `group_id`)
SELECT CONCAT('client_ip:127.0.0.1 user:admin operation:add_members group:Editors group_id:', g.`id`, ' count:1 members:manager'), 1, g.`id`
FROM `user_groups` g
WHERE g.`name` = 'Editors'
  AND NOT EXISTS (
    SELECT 1 FROM `log_groups` lg WHERE lg.`group_id` = g.`id` AND (lg.`event` LIKE '%operation:add_members%' OR lg.`event` LIKE '%added to group%')
