...
class EditUserController extends BaseController
{
    ...
    public function saveUser($edit_id): void
    {
        ...
        if (isset($_POST['perm_templ']) && Validation::is_number($_POST['perm_templ'])) {
            ...
            $user_details = get_user_detail_list_local($edit_id);
        ...
        if ($i_username != "" && $i_perm_templ > "0" && $i_fullname) {
            if (edit_user_local($edit_id, $i_username, $i_fullname, $i_email, $i_perm_templ, $i_description, $i_active, $i_password, $i_use_ldap)) {
    ...
    {
        $users = get_user_detail_list_local($edit_id);
        if (empty($users)) {
            $this->showError(_('User does not exist.'));
        }

        $user = $users[0];
        $edit_templ_perm = verify_permission_local('user_edit_templ_perm');
        ...
        $user_permissions = get_permissions_by_template_id_local($user['tpl_id']);
