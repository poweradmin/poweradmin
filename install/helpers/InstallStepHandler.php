...
class InstallStepHandler
{
    ...
    public function step4ConfiguringDatabase(array $errors): void
    {
        ...
        $inputData = [
            'db_user' => $this->input->request->get('db_user'),
            'db_pass' => $this->input->request->get('db_pass'),
            'db_host' => $this->input->request->get('db_host'),
            'db_port' => $this->input->request->get('db_port'),
            'db_name' => $this->input->request->get('db_name'),
            'db_charset' => $this->input->request->get('db_charset'),
            'db_collation' => $this->input->request->get('db_collation'),
            'db_type' => $this->input->request->get('db_type'),
    ...
    public function step6CreateLimitedRightsUser(array $errors): void
    {
        ...
        if ($credentials['db_type'] == 'sqlite') {
            ...
            $credentials['pa_db_user'] = $this->input->request->get('pa_db_user');
            $credentials['pa_db_pass'] = $this->input->request->get('pa_db_pass');
        }

        $pa_pass = $this->input->request->get('pa_pass');
        $hostmaster = $this->input->request->get('dns_hostmaster');
        $dns_ns1 = $this->input->request->get('dns_ns1');
        $dns_ns2 = $this->input->request->get('dns_ns2');
        $dns_ns3 = $this->input->request->get('dns_ns3');
        $dns_ns4 = $this->input->request->get('dns_ns4');
    ...
    public function step7CreateConfigurationFile(array $errors, ?string $defaultConfigFile = null): void
    {
        ...
        $db_port = ($this->input->request->get('db_type') == 'mysql' && $this->input->request->get('db_port') != 3306)
        || ($this->input->request->get('db_type') == 'pgsql' && $this->input->request->get('db_port') != 5432) ? $this->input->request->get('db_port') : '';
        ...
        $dns_ns4 = $this->input->request->get('dns_ns4');
        $db_host = $this->input->request->get('db_host');
        $db_user = $this->input->request->get('pa_db_user') ?? '';
        $db_pass = $this->input->request->get('pa_db_pass') ?? '';
        $db_name = $this->input->request->get('db_name');
        $db_type = $this->input->request->get('db_type');
        $db_charset = $this->input->request->get('db_charset');
        $db_collation = $this->input->request->get('db_collation');
    ...
    private function detectBaseUrlPrefix(): string
    {
        ...
        $scriptName = $this->input->server->get('SCRIPT_NAME', '');
