    ...
    <form action="<?= $_SERVER['PHP_SELF']; ?>?step=3<?=(defined('DEBUG') ? '&debug=true' : '') ?>" method="POST" name="db_settings">
        <table class="table table-bordered">
            ...
            <tr class="sqlite">
                ...
                <td><input type="text" class="form-control" name="dbFile" value="<?= getValue('dbFile', $_SERVER['DOCUMENT_ROOT']) ?>"></td>
            </tr>
            <tr class="mysql_pgsql">
                <td class="text-right"><label for="dbHost"><?= _('Host'); ?>:</label></td>
                <td><input type="text" class="form-control" name="dbHost" value="<?= getValue('dbHost', '127.0.0.1') ?>"></td>
            </tr>
            <tr class="mysql_pgsql">
                <td class="text-right"><label for="dbPort"><?= _('Port'); ?>:</label></td>
                <td><input type="text" class="form-control" name="dbPort" value="<?= getValue('dbPort', getDbPortDefault()) ?>"></td>
            </tr>
            <tr>
                <td class="text-right"><label for="dbUsername"><?= _('Username'); ?>:</label></td>
                <td><input type="text" class="form-control" name="dbUsername" value="<?= getValue('dbUsername') ?>"></td>
            </tr>
            <tr>
                <td class="text-right"><label for="dbPassword"><?= _('Password'); ?>:</label></td>
                <td><input type="password" class="form-control" name="dbPassword" value="<?= getValue('dbPassword') ?>"></td>
            </tr>
            <tr class="mysql_pgsql">
                <td class="text-right"><label for="dbDatabase"><?= _('Database'); ?>:</label></td>
                <td><input type="text" class="form-control" name="dbDatabase" value="<?= getValue('dbDatabase', 'poweradmin') ?>"></td>
            </tr>
            <tr class="mysql_pgsql">
        ...
        <?php if (isset($connectionStatus) && $connectionStatus && isset($requiredTables) && count($requiredTables) === 0): ?>
