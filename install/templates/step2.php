<?php include __DIR__ . '/header.php'; ?>
    ...
    <table class="table table-bordered">
        ...
        <?php foreach ($config['supportedDatabaseDrivers'] as $driver => $name): ?>
            <tr<?=in_array($driver, $config['availableSupportedDatabaseDrivers'], true) ? ' class="success"' : '' ?>>
                <td><?= $name ?></td>
                <td><?=in_array($driver, $config['availableSupportedDatabaseDrivers'], true) ? _('available') : _('not available') ?></td>
            </tr>
        <?php endforeach; ?>
        </tbody>
    </table>

<?php if ($configFileIsWritable && !in_array(false, $phpRequirements, true) && count($config['availableSupportedDatabaseDrivers']) > 0): ?>
    <?php $_SESSION['step'] = 3; ?>
...
<?php elseif (!$createConfigFile || !$configFileIsWritable): ?>
    <div class="alert alert-danger"><?= sprintf(_('Could not create Config-File "%s" or it is not writable!'), $config['configFile']); ?></div>
<?php else: ?>
    <div
