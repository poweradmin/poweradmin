    ...
    <form action="<?= $_SERVER['PHP_SELF']; ?>?step=4<?=(defined('DEBUG') ? '&amp;debug=true' : '') ?>" method="POST" name="db_settings">
        ...
        <table class="table table-bordered">
            ...
            <tr>
                ...
                <td><input type="text" class="form-control" name="fullname" value="<?= getValue('fullname') ?>"></td>
            </tr>
            <tr>
                <td class="text-right"><label for="email"><?= _('E-Mail'); ?>:</label></td>
                <td><input type="email" class="form-control" name="email" value="<?= getValue('email') ?>"></td>
            </tr>
            <tr>
                <td class="text-right"><label for="username"><?= _('Username'); ?>:</label></td>
                <td><input type="text" class="form-control" name="username" value="<?= getValue('username') ?>"></td>
            </tr>
            <tr>
                <td class="text-right"><label for="password"><?= _('Password'); ?>:</label></td>
                <td><input type="password" class="form-control" name="password" value="<?= getValue('password') ?>">
                </td>
            </tr>
            <tr>
                <td class="text-right"><label for="passwordRepeat"><?= _('Password Repeat'); ?>:</label></td>
                <td><input type="password" class="form-control" name="passwordRepeat" value="<?= getValue('passwordRepeat') ?>"></td>
            </tr>
            </tbody>
        </table>
