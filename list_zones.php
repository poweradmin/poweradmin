...
$count_zones_all_letterstart = zone_count_ng("all",LETTERSTART); 
...
if ($perm_view == "none") { 
	...
	if ($count_zones_all < ROWAMOUNT) {
		$zones = get_zones($perm_view,0,"all",ROWSTART,ROWAMOUNT);
	} else {
		$zones = get_zones($perm_view,0,LETTERSTART,ROWSTART,ROWAMOUNT);
