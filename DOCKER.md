...
- **Non-root start**: When the container starts as a non-root user (e.g., via `runAsUser`), it skips all privileged operations (`chown`, `chmod`) and listens on port **8080** by default. Volumes must be pre-configured as writable (see `fsGroup` below).
