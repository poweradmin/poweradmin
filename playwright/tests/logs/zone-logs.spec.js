...
test.describe('Zone Logs', () => {
  test.describe('Admin User', () => {
    test('should display zone logs page', async ({ adminPage: page }) => {
    ...
    test('should display logs count', async ({ adminPage: page }) => {
      await page.goto('/index.php?page=list_log_zones');
      const bodyText = await page.locator('body').textContent();
    ...
    test('should submit search form', async ({ adminPage: page }) => {
      ...
      await expect(page).toHaveURL(/list_log_zones/);
    });
    ...
    test('should display details button when logs exist', async ({ adminPage: page }) => {
      await page.goto('/index.php?page=list_log_zones');
  ...
  test.describe('Manager User - Permission Check', () => {
    test('should not have access to zone logs', async ({ managerPage: page }) => {
      ...
      expect(hasError || !page.url().includes('list_log_zones')).toBeTruthy();
    });
  });

  test.describe('Client User - Permission Check', () => {
    test('should not have access to zone logs', async ({ clientPage: page }) => {
      await page.goto('/index.php?page=list_log_zones');
      const bodyText = await page.locator('body').textContent();
      const hasError = bodyText.toLowerCase().includes('error') ||
                       bodyText.toLowerCase().includes('denied') ||
  ...
  test.describe('Viewer User - Permission Check', () => {
    test('should not have access to zone logs', async ({ viewerPage: page }) => {
      await page.goto('/index.php?page=list_log_zones');
      const bodyText = await page.locator('body').textContent();
      const hasError = bodyText.toLowerCase().includes('error') ||
                       bodyText.toLowerCase().includes('denied') ||
