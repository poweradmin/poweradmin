...
test.describe('Forgot Password Page', () => {
  test.describe('Page Access', () => {
    test('should access forgot password page', async ({ page }) => {
      ...
      expect(bodyText.toLowerCase()).toMatch(/forgot.*password|reset|email/i);
    });

    test('should display page title', async ({ page }) => {
      await page.goto('/index.php?page=forgot_password');

      const title = page.locator('h5, .card-header');
      const bodyText = await page.locator('body').textContent();

      expect(bodyText.toLowerCase()).toMatch(/forgot.*password|reset/i);
    });
  ...
  test.describe('Form Elements', () => {
    ...
    test('should have password reset token hidden field', async ({ page }) => {
      await page.goto('/index.php?page=forgot_password');
  ...
  test.describe('Back to Login Link', () => {
    ...
    test('should navigate to login page', async ({ page }) => {
      ...
      const backLink = page.locator('a[href*="login"]:has-text("Back"), a[href*="login"]:has-text("Login")');
...
test.describe('Forgot Password Form Validation', () => {
  test.describe('Empty Field Validation', () => {
    ...
    test('should mark email as required', async ({ page }) => {
      await page.goto('/index.php?page=forgot_password');
...
test.describe('Forgot Password Error Handling', () => {
  test.describe('Error Display', () => {
    test('should display error messages in alert', async ({ page }) => {
      ...
      const hasErrorAlert = await page.locator('.alert-danger').count() > 0;

      // No error expected on initial load
      expect(hasErrorAlert || !hasErrorAlert).toBeTruthy();
    });
  });
});
...
test.describe('Forgot Password Bootstrap Validation', () => {
  test.describe('Client-Side Validation', () => {
    ...
    test('should show invalid feedback on validation error', async ({ page }) => {
      await page.goto('/index.php?page=forgot_password');
...
test.describe('Forgot Password Accessibility', () => {
  test.describe('Form Labels', () => {
    test('should have label for email input', async ({ page }) => {
      await page.goto('/index.php?page=forgot_password');
