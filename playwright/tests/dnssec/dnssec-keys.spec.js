...
test.describe('DNSSEC Key Management', () => {
  ...
  test.describe('Delete DNSSEC Key', () => {
    ...
    test('should submit delete form without CSRF error', async ({ adminPage: page }) => {
      const zoneId = await ensureTestZoneExists(page, 'admin');
      expect(zoneId).toBeTruthy();
    ...
    test('viewer should not access DNSSEC management', async ({ viewerPage: page }) => {
      ...
      if (await row.count() > 0) {
        ...
        if (count > 0) {
          ...
          expect(bodyText.toLowerCase()).toMatch(/you do not have|access denied|not allowed|forbidden/i);
        }
      }
    });
  });
